...
public class GeneralBasic : MonoBehaviour {
	...
	void Start () {
        // Setup
        GameObject avatarRotate = UnityEngine.GameObject.Find("AvatarRotate");
        GameObject avatarScale = UnityEngine.GameObject.Find("AvatarScale");
        GameObject avatarMove = UnityEngine.GameObject.Find("AvatarMove");
	...
	void advancedExamples(){
        LeanTween.delayedCall(gameObject, 14f, ()=>{
			for(int i=0; i < 10; i++){
                ...
                // Instantiate Avatar
                GameObject dude = (GameObject)UnityEngine.GameObject.Instantiate(prefabAvatar, Vector3.zero, prefabAvatar.transform.rotation );
